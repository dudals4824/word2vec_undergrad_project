...
def run():
    traindf = pd.read_csv("./data/testtrain.csv")
    validationdf = pd.read_csv("./data/validation_test.csv")
    # frequency table : S
    f = open("./data/text8");
    line = f.readline();
    arr = line.split(" ");
...
with tf.device('/cpu:0'):
    ...
    W = tf.get_variable('w', shape=[cell_num, 14920], initializer=tf.contrib.layers.xavier_initializer())
    bias = tf.Variable(tf.constant(1.0, shape=[14920]))
