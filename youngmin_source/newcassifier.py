...
training_epochs = 1
batch_size = 64
train_len = pd.read_csv("./data/rt-polaritydata/data.csv").size / 2

keep_prob = tf.placeholder(tf.float32)

# 이전에 데이터 들어가는 부분이 있어야함.
# 여기서 None은 몇 라인이 들어갈지 정해질거임
X = tf.placeholder(tf.float32, [batch_size, 10, 300], name="x_input")
# X를 받는대로 reshape를 하면 단어 11개에 대해서 300차원으로 표현되고
# -1은 개수로 들어갈거임 None이랑 비슷한 의미
X_reshape = tf.reshape(X, [-1, 10, 300, 1])
...
with tf.name_scope("cost/loss") as scope:
    ...
    # cost = tf.reduce_mean(tf.nn.nce_loss(weights=tf.transpose(W5), biases=b5, inputs=L4, num_sampled=32, num_classes=50095, labels=Y))
...
traindf = pd.read_csv("./data/rt-polaritydata/newdata.csv")
...
for epoch in range(training_epochs):
    ...
    for i in range(0, total_batch):
        print(str(i+1)+"/"+str(total_batch))
        stime = time.time()
        start, batch_xs, batch_ys = cnnUtils.next_batch(start, batch_size, traindf, wv)
        print(str(time.time() - stime))
        feed_dict = {X: batch_xs, Y: batch_ys, keep_prob: 0.5}
        summary, c, acc, _ = sess.run([merged_summary, cost, accuracy, optimizer], feed_dict=feed_dict)
        writer.add_summary(summary, global_step=i)
        avg_cost += c / total_batch
        print(str(time.time() - stime))
    print('Epoch:', '%04d' % (epoch + 1), 'cost =', '{:.9f}'.format(avg_cost), 'accuarcy= ', '{:.9f}'.format(accuracy))
